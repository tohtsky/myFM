...
void ldiff_straightforward(Real x, Real y, Real &loss, Real &dx, Real &dy,
  ...
  Real denominator = Faddeeva::erf(x / OpS::SQRT2) - Faddeeva::erf(y / OpS::SQRT2);
  ...
  if (HessianTarget != nullptr) {
    (*HessianTarget)(label, label) +=
        -(OpS::SQRT2PI * x * denominator * expxx + 2 * expxx * expxx) / OpS::PI /
        denominator / denominator;
    (*HessianTarget)(label - 1, label - 1) +=
        -(-OpS::SQRT2PI * y * denominator * expyy + 2 * expyy * expyy) / OpS::PI /
...
TEST_CASE("minimization test.", "[probit-minimization]") {
  ...
  y << 0, 1, 0, 2, 1, 2;
  std::mt19937 rng(32);
  OpS sampler(x, y, 3, {0, 1, 2, 3, 4, 5}, rng); 
  sampler.alpha_0_reg = 0;
  sampler.alpha_reg = 0;
